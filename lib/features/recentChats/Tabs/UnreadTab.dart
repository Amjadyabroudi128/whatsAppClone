...
class _UnreadTabState extends State<UnreadTab> {
                      ...
                      builder: (context, snapshotUnread) {
                        ...
                        return Slidable(
                          startActionPane: ActionPane(
                            ...
                            children: [
                              _buildUnreadAction(chatRoomId, currentUserId, otherUserId),
                            ],
                          ),
                                ...
                                Flexible(
                                  child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                                    ...
                                    builder: (context, presenceSnap) {
                                      ...
                                      if (presenceSnap.hasData && presenceSnap.data!.exists) {
                                        final data = presenceSnap.data!.data()!;
                                        final bool receiverIsTyping = (data['isTypingTo'] as String?) == user!.uid;
                                        if (receiverIsTyping) {
                                          subtitle = "Typing";
                                        } else {
  ...
  Stream<List<String>> _getChatsWithUnreadMessages(List<Messages> messages, String currentUserId) {
    final chatRoomIds = messages.map((msg) {
      final isSender = msg.senderId == currentUserId;
      final otherUserId = isSender ? msg.receiverId : msg.senderId;
      return getChatRoomId(currentUserId, otherUserId!);
    }).toSet().toList();
    ...
    return FirebaseFirestore.instance
      ...
      for (var chatRoomId in chatRoomIds) {
        ...
        if (unreadQuery.docs.isNotEmpty) {
          chatsWithUnread.add(chatRoomId);
        }
      }
  ...
  Widget _buildUnreadAction(String chatRoomId, String currentUserId, String otherUserId) {
    return StreamBuilder<QuerySnapshot>(
